import type { Substance, SubstanceTag, OrganHealth } from '../data/types';
...
export function computeConditionWarnings(
  ...
  const patientConditions = Array.from(activeConditions)
  ...
  // 6. Dynamic Condition Database Rules
  for (const cond of patientConditions as any[]) {
      if (cond.contraindicatedTags && cond.contraindicatedTags.length > 0) {
          for (const s of substances) {
              const violatingTags = cond.contraindicatedTags.filter((tag: SubstanceTag) => has(s, tag));
              if (violatingTags.length > 0) {
                  addSingle(
                      ...
                      s,
                      'contraindicated',
                      `Condition Contraindication: ${cond.name}`,
                      `The patient's condition (${cond.name}) is strictly contraindicated with substances carrying the following properties: ${violatingTags.join(', ')}.`,
                      `Avoid concurrent use. Evaluate for alternative therapies that do not possess ${violatingTags[0]} properties.`,
                      true
                  );
              }
          }
