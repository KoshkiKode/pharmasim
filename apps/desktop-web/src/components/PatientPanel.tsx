...
import type { PatientProfile, MetabolizerPhenotype, OrganHealth } from '@pharmasim/engine';
      ...
      <div ref={condRef} className="mt-5 border-t border-border pt-4">
        ...
        {patient.conditions.length > 0 && (
          <div className="mt-3 flex flex-col gap-2">
            <div className="flex flex-wrap gap-1.5" data-testid="selected-conditions">
            ...
            {patient.conditions.some(id => ALL_CONDITIONS.find(c => c.id === id)?.pkModifiers?.justification) && (
              <div className="mt-2 rounded-lg bg-sev-minor/5 border border-sev-minor/20 p-3">
                <h4 className="text-[11px] font-semibold text-sev-minor uppercase tracking-wider mb-2">Physiological Modifiers Applied</h4>
                <ul className="space-y-2">
                  {patient.conditions.map(id => {
                    const cond = ALL_CONDITIONS.find(c => c.id === id);
                    if (!cond || !cond.pkModifiers?.justification) return null;
                    return (
                      <li key={`justification-${id}`} className="text-[11px] leading-relaxed text-ink-muted">
                        <span className="font-semibold text-ink">{cond.name}:</span> {cond.pkModifiers.justification}
                      </li>
                    );
                  })}
                </ul>
